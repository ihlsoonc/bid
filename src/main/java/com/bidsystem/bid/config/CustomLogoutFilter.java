...
public class CustomLogoutFilter extends GenericFilterBean {
    ...
    private void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (!requestMethod.equals("POST")) {
            logger.info("Request method is not POST. Passing through filter chain.");
            filterChain.doFilter(request, response);
            return;
        }
        ...
        if (refreshToken == null) {
            ...
            response.getWriter().write("{\"error_code\": \"REFRESH_INVALID\", \"message\": \"Refresh token is invalid(null). Please login.\"}");
        ...
        try {
            ...
            response.setContentType("application/json");
            response.getWriter().write("{\"error_code\": \"REFRESH_INVALID\", \"message\": \"Refresh token is invalid(expired). Please login.\"}");
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        ...
        if (!category.equals("Refresh")) {
            ...
            response.setContentType("application/json");
            response.getWriter().write("{\"error_code\": \"REFRESH_INVALID\", \"message\": \"Refresh token is invalid(category). Please login.\"}");
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        ...
        if (!isExist) {
            ...
            response.setContentType("application/json");
            response.getWriter().write("{\"error_code\": \"REFRESH_INVALID\", \"message\": \"Refresh token is not in DB. Please login.\"}");
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
